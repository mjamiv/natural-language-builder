...
def _generate_materials_script(materials: list[dict], sections: list[dict]) -> str:
    ...
    for m in materials:
        ...
        if mtype == "Concrete01":
            ...
            if core and cover and steel:
                ...
                nfr = core.get("nfr", 8)
                nft = core.get("nft", 16)
                lines.append(f"ops.patch('circ', {mat_core}, {nfr}, {nft}, 0.0, 0.0, 0.0, {r_core:.2f}, 0.0, 6.283185307)")
    ...
    for m in fiber_sections:
        ...
        if core and cover and steel:
            ...
            mat_core = core.get("material", tag + 1)
            r_core = core.get("radius", 24.0)
            lines.append(f"ops.patch('circ', {mat_core}, {core.get('nfr', 8)}, {core.get('nft', 16)}, 0.0, 0.0, 0.0, {r_core:.2f}, 0.0, 6.283185307)")
            mat_cover = cover.get("material", tag + 2)
            r_out = cover.get("outer_radius", r_core + 3)
            r_in = cover.get("inner_radius", r_core)
            lines.append(f"ops.patch('circ', {mat_cover}, {cover.get('nfr', 2)}, {cover.get('nft', 16)}, 0.0, 0.0, {r_in:.2f}, {r_out:.2f}, 0.0, 6.283185307)")
    ...
    for s in sections:
        ...
        if stype == "circular_rc":
            ...
            if core and cover and steel:
                ...
                lines.append(f"ops.patch('circ', {mat_cover}, {nfr_c}, {nft_c}, 0.0, 0.0, {r_in:.2f}, {r_out:.2f}, 0.0, 6.283185307)")
                # Steel layer
